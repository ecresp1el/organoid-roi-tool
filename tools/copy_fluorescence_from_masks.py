...
@dataclass
class JobOutcome:
    """Book-keeping structure describing what happened for a single mask.

    The attributes allow callers (and CLI logs) to distinguish between:
    * masks that had no matching fluorescence source (``fluorescence_source`` is ``None``),
    * masks that were skipped because outputs already existed, and
    * masks that produced fresh artefacts (``copied_raw`` / ``saved_masked`` / ``saved_cropped``).
    """
    mask_path: Path
    ...
    @property
    def ok(self) -> bool:
        """Return ``True`` when the mask led to actual processing instead of a skip."""
        return self.skipped_reason is None


SUFFIX_MASK = "_mask.tif"  # Brightfield mask naming produced by the ROI GUI
TIFF_EXTS = (".tif", ".tiff", ".TIF", ".TIFF")  # Fluorescence exports may vary in case and extension


def iter_masks(project_root: Path) -> Iterable[Path]:
    """Iterate through the project tree and yield every ROI mask path.

    Only files ending with ``_mask.tif`` below ``project_root / "wells"`` are returned.
    Derivative outputs such as ``*_roi_masked.tif`` and ``*_roi_masked_cropped.tif`` are ignored
    so that each timepoint is handled exactly once.
    """
...
def read_tiff_array(path: Path) -> np.ndarray:
    """Load a TIFF into a numpy array with tifffile, falling back to Pillow if needed.

    Organoid exports often rely on LZW compression, which ``tifffile`` normally decodes via
    ``imagecodecs``. Environments without that wheel can still proceed because we defer to
    ``Pillow`` whenever the error message explicitly names the missing codec. The Pillow code
    path may drop some TIFF metadata but preserves pixel values, which is sufficient for masking.
    """
    try:
...
def load_mask(mask_path: Path) -> np.ndarray:
    """Return the ROI mask as a boolean array using a >0 threshold.

    Mask files generated by the GUI are grayscale ``uint8`` images where inside-ROI pixels are
    255 and the background is 0. Thresholding with ``> 0`` keeps the function robust to stray
    non-zero values without caring about the exact intensity.
    """
...
def apply_mask(data: np.ndarray, mask_bool: np.ndarray) -> np.ndarray:
    """Apply the 2-D ROI mask to a 2-D image or any stack of frames/channels.

    The function broadcasts the boolean mask across all leading axes, so fluorescence z-stacks or
    multi-channel images are handled without additional logic. Pixels outside the ROI are set to 0
    (the dtype-specific representation of black).
    """
    if data.ndim == 2:
...
def crop_to_mask(data: np.ndarray, mask_bool: np.ndarray) -> np.ndarray:
    """Tight crop the masked data to the ROI bounding box along the final (y, x) axes.

    The coordinate order matches NumPy conventions: the slice is applied to the last two axes so
    any leading dimensions (e.g., channels or z-planes) are preserved verbatim.
    """
...
def resolve_source(base_name: str, fluor_root: Path) -> Optional[Path]:
    """Return the fluorescence TIFF whose stem matches ``base_name`` in the flat folder.

    The function checks common extensions in both lowercase and uppercase forms. Only the first
    match is returned because the fluorescence export directory is assumed to be flat and unique.
    """
...
def process_mask(
    ...
    """Handle one mask: locate the source fluorescence file and generate derivatives.

    When ``dry_run`` is false and a matching fluorescence image exists, the function copies the
    raw TIFF, writes a masked version, and writes a cropped variant inside ``target_subdir``. Any
    mismatches, missing sources, or pre-existing outputs are recorded in the returned ``JobOutcome``.
    """
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    """Configure and parse CLI options for the fluorescence copy workflow.

    Exposes knobs for channel labelling, destination subfolder naming, overwrite behaviour,
    and utility switches (``--dry-run`` / ``--limit``) so the script can be rehearsed safely.
    """
...
def main(argv: Optional[list[str]] = None) -> int:
    """Run the end-to-end synchronization between masks and fluorescence images.

    Validates user inputs, iterates over every detected mask, reports per-file status, and prints
    a concise summary that doubles as an execution log for record keeping. Returns the process
    exit code expected by ``if __name__ == "__main__"`` blocks.
    """
