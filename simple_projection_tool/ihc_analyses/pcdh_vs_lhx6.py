...
class PCDHvsLHX6_WTvsKOIHCAnalysis(ProjectionAnalysis):
    ...
    1. **Importing** all TIFFs produced by ``simple_channel_projections`` under
       ``<base_path>/simple_projections`` and determining whether each file
    ...
    2. **Processing** every 16-bit projection by loading the pixels into
       ...
       count, and a 95 % confidence interval for the mean).
    ...
    4. **Plotting** group-level distributions using matched conventions
       (boxplots and mean±SEM bar charts saved as SVG + PNG with Arial text) to
    ...
    def run_statistics(self) -> None:
        ...
        summary_projection_types = (
            summary["projection_type"].nunique() if "projection_type" in summary.columns else 0
        )
        print(
        ...
        if comparisons.empty:
            ...
            comparison_projection_types = (
                comparisons["projection_type"].nunique()
                if "projection_type" in comparisons.columns
                else 0
            )
            print(
    ...
    def generate_plots(self) -> None:
        ...
        # Ensure exported SVG keeps editable text when opened in Illustrator.
        ...
        for projection_type, projection_df in self.results.groupby("projection_type"):
            ...
            if figure is None:
                ...
                continue
            description = (
                "Pixel mean distribution (box + mean±SEM) for WT vs KO projections"
            )
    ...
    def _plot_projection_summary(
        self, projection_type: str, projection_df: pd.DataFrame
    ) -> Optional[plt.Figure]:
        ...
        sems = [float(np.std(arr, ddof=1) / np.sqrt(arr.size)) if arr.size > 1 else 0.0 for arr in values]
        ...
        axes[1].set_xticks(x, labels=groups)
